  ...
  async createDataPoint(ident, common, types, value, extend, native) {
    ...
    try {
      ...
      if (!obj) {
        ...
        let ischange = false;
        this.log.info(`common: ${JSON.stringify(common)}`);
        this.log.info(`obj.common: ${JSON.stringify(obj.common)}`);
        this.log.info(`common: ${JSON.stringify(Object.keys(common).length)}`);
        this.log.info(`obj.common: ${Object.keys(obj.common || {}).length}`);
        if (extend) {
          let countStates = 0;
          if (obj.common && common && common.states == null && obj.common.states != null) {
            countStates = 1;
          }
          this.log.debug(`countStates: ${countStates}`);
          if (Object.keys(common).length > Object.keys(obj.common || {}).length - countStates) {
            ischange = true;
          } else {
            for (const key in common) {
              this.log.info(`key: ${JSON.stringify(obj.common[key])}`);
              this.log.info(`key: ${JSON.stringify(common[key])}`);
